...
- **Made by Many (by Table Talks)** — a separate community/expedition platform, currently centred on **"Mission 001: Siargao"** (Philippines) — a documentary-style expedition preserving unwritten local recipes, stories and traditions, funded via a public-ledger transparency model ("Your funds don't matter. Your participation does."). This lives at `/missions` and is visibly **work-in-progress**: it has placeholder testimonial cards (`[ insert ]` text), a `$230 AUD raised / 6 families` counter, and blank avatar art. Treat this section as "coming soon" content, not finished copy — don't polish it into something it isn't yet.
